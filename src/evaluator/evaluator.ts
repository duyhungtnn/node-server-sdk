import { createUser, Evaluator, Feature, SegmentUser, SegmentUsers } from '@bucketeer/node-evaluation';
...
interface EvaluateLocally {
  evaluate(user: User, featureID: string): Promise<Evaluation>;
}

class NodeEvaluator implements EvaluateLocally {
  private tag: string;
  ...
  async evaluate(user: User, featureID: string): Promise<Evaluation> {
    ...
    const userEvaluations = await evaluator.evaluateFeatures(targetFeatures, protoUser, segmentUsersMap, this.tag);
